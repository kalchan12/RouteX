...
function mkLane(id: string, roadId: string, index: number, waypoints: {x:number,y:number}[]): Lane {
  let len = 0;
  ...
  return { id, roadId, index, direction: 'forward', length: len, speedLimit: 13.9, waypoints, connections: [] };
}

export function createDefaultScenario(): Scenario {
  // Lanes: E-W gets 2 lanes per direction
  ...
  // Connections
  eIn0.connections = [{toLaneId: 'e-out-0', toRoadId: 're-out', turnType: 'straight'}, {toLaneId: 's-out', toRoadId: 'rs-out', turnType: 'right'}];
  eIn1.connections = [{toLaneId: 'e-out-1', toRoadId: 're-out', turnType: 'straight'}];
  
  wIn0.connections = [{toLaneId: 'w-out-0', toRoadId: 'rw-out', turnType: 'straight'}, {toLaneId: 'n-out', toRoadId: 'rn-out', turnType: 'right'}];
  wIn1.connections = [{toLaneId: 'w-out-1', toRoadId: 'rw-out', turnType: 'straight'}];

  nIn.connections = [{toLaneId: 'n-out', toRoadId: 'rn-out', turnType: 'straight'}, {toLaneId: 'e-out-0', toRoadId: 're-out', turnType: 'right'}];
  sIn.connections = [{toLaneId: 's-out', toRoadId: 'rs-out', turnType: 'straight'}, {toLaneId: 'w-out-0', toRoadId: 'rw-out', turnType: 'right'}];
  ...
  const ix: Intersection = {
    id: 'ix', position: {x: 0, y: 0}, size: ISZ,
    phases: [
    ...
    currentPhase: 0, phaseTimer: 0, inYellow: false, lights
  };

  const spawners: VehicleSpawner[] = [
    { laneId: 'e-in-0', rate: 20, routes: [], typeWeights: [{type: VehicleType.Car, weight: 0.9}, {type: VehicleType.Truck, weight: 0.1}] },
    { laneId: 'e-in-1', rate: 10, routes: [], typeWeights: [{type: VehicleType.Car, weight: 1.0}] },
    { laneId: 'w-in-0', rate: 15, routes: [], typeWeights: [{type: VehicleType.Car, weight: 0.8}, {type: VehicleType.Bus, weight: 0.2}] },
  ...
  return { name: 'Four-Way Multi-Lane', description: 'Intersection with multi-lane E-W roads.', roads, intersections: [ix], spawners, pedestrians, seed: 42 };
}
